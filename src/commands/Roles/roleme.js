...
const avoidRoles = ['499420996067917825', '499420999352188930', '472540827357544458'];
module.exports = class extends Command {
	...
	async add(msg, [role]) {
		...
		if ((role.id === '499420996067917825' && msg.member.roles.has('499420999352188930')) || (role.id === '499420999352188930' && msg.member.roles.has('499420996067917825'))) {
			return msg.send('｢ **Error** ｣ You can only have *one* region role.');
		} 
		if (!avoidRoles.includes(role.id)) {
	...
	roleCheck(msg) {
		...
		for (var i in memberRoles) {
			if (avoidRoles.includes(memberRoles[i])) continue;
